    ...
    suggestedUsers.innerHTML = users.map(user => {
        ...
        return `
            <div class="suggested-user">
                ...
                <div class="suggested-user-actions" style="display: flex; gap: 5px;">
                    ...
                    <button class="dm-btn" 
                            style="background: #3b82f6; color: white; border: none; padding: 5px 10px; border-radius: 15px; font-size: 12px; cursor: pointer;"
                            onclick="openDMModal('${user.id}', '${escapeHtml(user.username)}')">
                        💬
                    </button>
                </div>
            </div>
...
// Open DM Modal to send a message (#23)
window.openDMModal = function(receiverId, receiverUsername) {
    if (!currentUser) {
    ...
    modal.style.cssText = 'background:white;padding:25px;border-radius:15px;max-width:400px;width:90%;color:#333;';
    modal.innerHTML = `
        <h3 style="margin-bottom:15px;color:#ff6b1a;">Send Message to ${receiverUsername}</h3>
        <textarea id="dmMessageContent" rows="4" style="width:100%;padding:10px;border:1px solid #e5e7eb;border-radius:8px;resize:vertical;margin-bottom:15px;" placeholder="Type your message..."></textarea>
        <div style="display:flex;gap:10px;justify-content:flex-end;">
            <button id="cancelDM" style="padding:10px 20px;border:1px solid #e5e7eb;background:white;border-radius:8px;cursor:pointer;">Cancel</button>
            <button id="sendDM" style="padding:10px 20px;background:#ff6b1a;color:white;border:none;border-radius:8px;cursor:pointer;">Send</button>
        </div>
    `;
    ...
    modal.querySelector('#cancelDM').onclick = () => overlay.remove();
    overlay.onclick = (e) => { if (e.target === overlay) overlay.remove(); };

    modal.querySelector('#sendDM').onclick = async () => {
        const content = document.getElementById('dmMessageContent').value.trim();
        if (!content) {
...
function initLiveStream() {
    ...
    if (!player || streamBtns.length === 0) return;

    // Stream button click handlers
    streamBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            const videoId = btn.dataset.video;
            const title = btn.dataset.title;
            const startTime = btn.dataset.start || 0;

            // Update active state
            streamBtns.forEach(b => b.classList.remove('active'));
            btn.classList.add('active');

            // Change stream (with optional start time)
            player.src = `https://www.youtube.com/embed/${videoId}?autoplay=1&mute=1&loop=1&playlist=${videoId}&controls=1&start=${startTime}`;
    ...
    setInterval(() => {
        if (viewersEl) {
            const currentViewers = parseInt(viewersEl.textContent.replace(/,/g, '')) || 8000;
            const change = Math.floor(Math.random() * 400) - 200;
            const newViewers = Math.max(5000, currentViewers + change);
            viewersEl.textContent = newViewers.toLocaleString();
        }
    }, 4000);
}
